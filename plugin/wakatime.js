...
var STATE_FILE = path2.join(os2.homedir(), ".wakatime", "opencode.json");
function readState() {
  try {
    const content = fs2.readFileSync(STATE_FILE, "utf-8");
...
function writeState(state) {
  try {
    const dir = path2.dirname(STATE_FILE);
...
function processHeartbeat(projectFolder, force = false) {
  ...
  const hooks = {
    ...
    event: async ({ event }) => {
      if (event.type === "session.deleted" || event.type === "session.idle") {
        ...
        processHeartbeat(projectFolder, true);
      }
    }
  };
  return hooks;
